---
name: why
description: 현상의 근본 원인 파악 — 4개 서브에이전트 병렬 조사 후 소크라테스식 추론으로 원칙 수준 근본 원인 도달, 그리고 그 교훈을 '테스트(검증 가능)' 또는 '룰(판단)'로 라우팅
allowed-tools:
  - Read
  - Glob
  - Grep
...
### 4.5. 라우팅 — 테스트로 둘까, 룰로 둘까

근본 원인이 확정되면, 그 교훈을 **어디에 둘지** 먼저 정한다. 룰을 무한정 쌓으면 매 세션 컨텍스트를 먹고(토큰 비용·context rot) 권고일 뿐 강제되지 않는다. 그래서 한 가지를 묻는다:

> **이 근본 원인을 기계로 검증할 수 있는가?** (테스트·린트·타입체크·빌드·grep 등으로 자동 판정 가능한가)

- **검증 가능 → 테스트 경로 (Step 5-T).** 컨텍스트에 룰을 쌓지 않고, 어기면 빨간불이 뜨는 테스트/게이트로 강제한다. 테스트는 필요할 때만 실행되고 실패가 곧 피드백이라, 룰보다 강하고 토큰 효율적이다.
- **검증 불가 → 룰 경로 (Step 5).** 아키텍처 선택·네이밍·톤·"왜 X를 폐기했나" 같은 판단/취향은 테스트로 못 짠다. 이런 것만 룰로 둔다.

판정 가이드:
- ✅ 테스트로: "이 회귀 재발 금지", "함수 X는 빈 입력에 Y 반환", "빌드·타입체크 통과", "금지 패턴 grep 0건"
- ✅ 룰로: "왜 라이브러리 X 폐기", "이 프로젝트는 Z 아키텍처를 따른다", "톤·네이밍 규약" 같은 비자동 판단

경계가 애매하거나 둘 다 필요하면 `AskUserQuestion`으로 **테스트 / 룰 / 둘 다**를 선택받는다. 기본값은 "검증 가능하면 테스트".

### 5. (룰 경로) rules 파일 저장 — *라우팅에서 '룰'로 결정된 경우에만*
...
### 5-T. (테스트 경로) 테스트/게이트로 강제

룰 파일을 만들지 않는다. 대신 근본 원인을 **실패하는 테스트(red)** 로 인코딩한다.

1. 프로젝트의 테스트 프레임워크를 탐지한다 (`package.json`/`pyproject.toml`/기존 `test*` 디렉토리 등). 없으면 최소 체크 스크립트나 hamstern stop 훅 게이트를 제안한다.
2. 근본 원인을 재현하는 **실패 테스트**를 작성/제안한다. 테스트 이름·주석에 "왜 이게 존재하는가(근본 원인)"를 한 줄 남긴다.
3. 에이전트가 자유롭게 구현하되 이 테스트를 **반드시 통과**시키게 한다. ⚠️ 테스트를 약화시키거나 입력을 특수처리하는 치팅을 금지하고, 실패 시 "무엇이·왜 깨졌는지"를 다음 시도의 컨텍스트로 돌려준다(맹목 재시도 금지).
4. 기록은 `{project_root}/.hamstern/why/tests.md` 에 한 줄 append: `- {YYYY-MM-DD} {topic}: {테스트 경로} — {근본 원인 한 줄}`. (룰처럼 매 세션 로드되지 않음 — 강제는 테스트가 한다.)
...
🧪 테스트로 강제: {테스트 경로}
   근본 원인 : {원칙 한 문장}
   기록      : .hamstern/why/tests.md
▶ 컨텍스트를 늘리지 않고, 어기면 빨간불로 막힙니다.
```
